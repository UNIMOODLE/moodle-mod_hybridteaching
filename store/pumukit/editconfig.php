...
use hybridteachstore_pumukit\configs;
...
$url = new moodle_url('/mod/hybridteaching/store/pumukit/editconfig.php');
$PAGE->set_url($url);
...
if (!empty($configid)) {
    ...
    $pumukitconfig = configs::load_config($config->subpluginconfigid);
    unset($pumukitconfig->id);
    $config = (object) array_merge((array) $config, (array) $pumukitconfig);
}

$mform = new htpumukit_config_edit_form(null, [$config, $type]);
$message = '';
$error = '';
