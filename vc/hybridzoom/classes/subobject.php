...
class subobject{
   ...
   protected $zoom;
    ...
    public function __construct($id){
        ...
        $this->zoom=$DB->get_records('hybridteaching_zoom',array('hybridteachingid'=> $id));
        //$this->zoom->type="zoom";
    ...
    function get_zone_access($hybridteachingid){
        ...
        $vc=$DB->get_record_sql("SELECT * FROM {hybridteaching_zoom} WHERE hybridteachingid=:id AND (starttime+duration>=UNIX_TIMESTAMP() OR starttime IS NULL) ORDER BY starttime DESC LIMIT 1", array('id'=>$hybridteachingid));
