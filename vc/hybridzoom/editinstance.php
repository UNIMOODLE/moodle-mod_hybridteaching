...
$url = new moodle_url('/mod/hybridteaching/vc/hybridzoom/editinstance.php');
$PAGE->set_url($url);
...
if (!empty($instanceid)) {
    ...
    $zoominstanceid = $DB->get_field('hybridteaching_instances', 'subplugininstanceid', ['id' => $instanceid]);
    $zoominstance = instances::load_instance($zoominstanceid);
    unset($zoominstance->id);
    $instance = (object) array_merge((array) $instance, (array) $zoominstance);
}

$mform = new hybridzoom_instance_edit_form(null, array($instance, $type));
$message = '';
$error = '';
