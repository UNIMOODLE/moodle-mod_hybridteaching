...
use mod_hybridteaching\plugininfo\hybridteachingvc;
...
$url = new moodle_url('/mod/hybridteaching/vc/hybridbbb/editinstance.php');
$PAGE->set_url($url);
...
$instance = null;
if (!empty($instanceid)) {
    $instance = instances::load_instance($instanceid);
    $bbbinstanceid = $DB->get_field('hybridteaching_instances', 'subplugininstanceid', ['id' => $instanceid]);
    $bbbinstance = instances::load_instance($bbbinstanceid);
    unset($bbbinstance->id);
    $instance = (object) array_merge((array) $instance, (array) $bbbinstance);
}

$mform = new hybridbbb_instance_edit_form(null, array($instance, $type));
$message = '';
$error = '';
