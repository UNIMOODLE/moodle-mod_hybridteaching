...
require_once($CFG->dirroot.'/mod/hybridteaching/vc/hybridzoom/locallib.php');
...
define('HYBRIDZOOM_API_URL', 'https://api.zoom.us/v2/');
...
//TO-DO: cambiar esto por la configuración de zoom específica        
        //$config = get_config('isyczoomav');
        ...
        try{
            ...
            foreach ($requiredfields as $requiredfield) {
                if (!empty($config->$requiredfield)) {
                    ...
                    throw new moodle_exception('zoomerr_field_missing', 'hybridzoom', '', get_string($requiredfield, 'hybridzoom'));
                }
            }*/
            
            // Get and remember the API URL.
            $this->apiurl = HYBRIDZOOM_API_URL;
            
        } catch (moodle_exception $exception) {
            throw new moodle_exception('errorwebservice', 'hybridzoom', '', $exception->getMessage());
        }
    ...
    protected function _make_call($path, $data = array(), $method = 'get') {
        ...
        if ($curl->get_errno()) {
            throw new moodle_exception('errorwebservice', 'hybridzoom', '', $curl->error);
        }
        ...
        if ($httpstatus >= 400) {
            if ($response) {
                throw new moodle_exception('errorwebservice', 'hybridzoom', '', get_string('errorwebservice', 'hybridteachingvc_hybridzoom',$response->message) );
            } else {
                throw new moodle_exception('errorwebservice', 'hybridzoom', '', "HTTP Status $httpstatus");
            }
        }
    ...
    protected function _make_paginated_call($url, $data = array(), $datatoget) {
        $aggregatedata = array();
        $data['page_size'] = HYBRIDZOOM_MAX_RECORDS_PER_CALL;
    ...
    public function get_user($identifier) {
        ...
        try {
            ...
            if (hybridzoom_is_user_not_found_error($error->getMessage())) {
                return false;
            } else {
                throw $error;
            }
    ...
    public function get_roles() {
        ...
        try {
            ...
            if (hybridzoom_is_roles_not_found_error($error->getMessage())) {
                return false;
            } else {
                throw $error;
            }
    ...
    public function get_users(){
        ...
        try {
            ...
            if (hybridzoom_is_users_not_found_error($error->getMessage())) {
                return false;
            } else {
                throw $error;
            }
    ...
    protected function _database_to_api($zoom) {
        ...
        */
        if (isset($zoom->initialrecord) && $zoom->initialrecord==1){
            $data['auto_recording'] = HYBRIDZOOM_RECORDING_CLOUD;
        }
        if (isset($zoom->userecordvc) && $zoom->userecordvc==0) {
            $data['auto_recording'] = HYBRIDZOOM_RECORDING_DISABLED;
        }

        if (isset($zoom->webinar) && $zoom->webinar) {
            //opción "Permitir acceso en cualquier momento", depende de variable undatedsession
            $data['type'] = $zoom->undatedsession ? HYBRIDZOOM_RECURRING_WEBINAR : HYBRIDZOOM_SCHEDULED_WEBINAR;
        } else {
            $data['type'] = $zoom->undatedsession ? HYBRIDZOOM_RECURRING_MEETING : HYBRIDZOOM_SCHEDULED_MEETING;
        }
        if ($data['type'] == HYBRIDZOOM_SCHEDULED_MEETING || $data['type'] == HYBRIDZOOM_SCHEDULED_WEBINAR) {
    ...
    */
    ...
    public function get_user_report($userid, $from, $to) {
        ...
        $data = array('from' => $from, 'to' => $to, 'page_size' => HYBRIDZOOM_MAX_RECORDS_PER_CALL);
    ...
    protected function get_access_token() {
        $cache = cache::make('hybridteachingvc_hybridzoom', 'oauth');
        //$cache = cache::make('mod_isyczoomav', 'oauth');
    ...
    private function oauth($cache) {
        ...
        if (isset($response->access_token)) {
            ...
            throw new moodle_exception('errorwebservice', 'hybridzoom', '', get_string('zoomerr_no_access_token', 'hybridzoom'));
        }
    }
