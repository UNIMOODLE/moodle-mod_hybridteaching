...
$event = \mod_hybridteaching\event\course_module_instance_list_viewed::create(array(
    'context' => $coursecontext
));
$event->add_record_snapshot('course', $course);
$event->trigger();

$PAGE->set_url('/mod/hybridteaching/index.php', array('id' => $id, 'h' => $hybridteachingid));
...
foreach ($hybridteachings as $hybridteaching) {
    if (!$hybridteaching->visible) {
        $link = html_writer::link(
            new moodle_url('/mod/hybridteaching/view.php', array('id' => $hybridteaching->coursemodule, 'h' => $hybridteaching->id)),
    ...
    if ($course->format == 'weeks' || $course->format == 'topics') {
        $table->data[] = array($hybridteaching->section, $link);
    } else {
